...
namespace Html2Sql.Controllers
{
    ...
    public class MembersDetailsController : ControllerBase
    {
        ...
        {
            if (title.Contains("رئیس"))
                return BoardType.President;
            if (title.Contains("نائب رئیس اول"))
                return BoardType.VicePresident;
            if (title.Contains("نائب رئیس دوم"))
                return BoardType.SecondVicePresident;
        ...
        private List<RawMember> GetRawMembers()
        {
            ...
            var html = new StreamReader(
                @"C:\Users\muhammadS\Desktop\majles\all_members\index.html"
            );
            hdoc.Load(html);
            html.Close();
            //members
            ...
            //boards
            var yearIds = hdoc.GetElementbyId("pills-tab")
                .QuerySelectorAll("a")
                .Select(
                    x =>
                        new
            ...
            foreach (var year in yearIds)
            {
                ...
                {
                    var url =
                        base_url + member.QuerySelector("a").GetAttributeValue("href", "") ?? "";
                    var title = member.QuerySelector(
                        "div.members__title.text__14.text__medium > span"
                    );
        ...
        public async Task<List<RawMember>> GetAsync()
        {
            ...
            var sw1 = new StreamWriter(
                @"C:\Users\muhammadS\Desktop\majles\all_members\parsed.json"
            );
            sw1.Write(parsed);
            sw1.Close();
            ...
            {
                using (
                    var sw2 = new StreamWriter(
                        $"C:\\Users\\muhammadS\\Desktop\\majles\\all_members\\pages\\{item.index}.html"
                    )
                )
                {
        ...
        {
            if (do_drop)
                _context.Database.DropCollection("membersDetails");
            ...
            foreach (var raw in t.Yaap(total: t.Count))
            {
                ...
                var hists = raw.BoardYear
                    .Zip(raw.BoardType)
                    .Select(x => new BoardMember { BoardYear = x.First, BoardType = x.Second });
                ...
                member.ChooseRegion = hdoc.QuerySelector(".agent-data__position")
                    .InnerText.Replace("نماینده", "")
                    .s_();
                var dataDict_tmp = hdoc.QuerySelectorAll(".agent-data__list .item")
                    .Select(x => new { k = x.css2text(".label"), v = x.css2text(".data") })
                    .ToList();
                ...
                if (hist != "")
                {
                    var histsAll = hist.Replace("دوره", "")
                        .s_()
                        .Split(" ")
                        .Select(x => utils.persianNum2int(x.s_()))
                        .ToList();
                ...
                for (int i = 0; i < memberships.Count; i++)
                {
                    ...
                    var items = membership
                        .QuerySelectorAll(".group-sec__item")
                        .Select(
                            x =>
                                new Membership
                                {
                                    ...
                                    Year =
                                        i == 0
                                            ? utils.persianNum2int(
                                                x.css2text(".group-sec__subtitle")
                                                    .Replace("سال", "")
                                                    .Replace("دوره", "")
                                            )
                                            : 0,
                                    ...
                                    Url =
                                        base_url
                                        + x.QuerySelector("a").GetAttributeValue("href", ""),
                                }
                        );
        ...
        public async Task<int> GetNews()
        {
            ...
            foreach (var raw in t.Yaap(total: t.Count))
            {
                ...
                if (news != null)
                {
                    var d = news.QuerySelectorAll(".news-collection__header")
                        .Select(
                            x =>
                                new
                                {
                                    name = x.QuerySelector(".news-collection__main-title")
                                        .InnerText.s_(),
                                    url = base_url
                                        + x.QuerySelector("a").GetAttributeValue("href", "")
                                }
                    ...
                    if (speeches_url != "")
                    {
                        using (
                            var sw = new StreamWriter(
                                $"C:\\Users\\muhammadS\\Desktop\\majles\\all_members\\speeches\\{raw.index}.html"
                            )
                        )
                        {
                            try
                        ...
                        if (news_url != "")
                        {
                            using (
                                var sw = new StreamWriter(
                                    $"C:\\Users\\muhammadS\\Desktop\\majles\\all_members\\news\\{raw.index}.html"
                                )
                            )
                            {
                                try
        ...
        public async Task<int> AddNewsSpeeches()
        {
            Console.Clear();
            //news
            var ls = Directory.GetFiles(
                $"C:\\Users\\muhammadS\\Desktop\\majles\\all_members\\news"
            );
            foreach (
                var i in ls.Yaap(
                    settings: new YaapSettings
                    {
                        Description = "news",
                        Width = 100,
            ...
            )
            {
                ...
                var news = hdoc.QuerySelectorAll(".news-item__content")
                    .Select(
                        x =>
                            new News_Speeches
                            {
                                ...
                                Url =
                                    base_url
                                    + x.QuerySelector("a.news-item__title")
                ...
                var update = Builders<MemeberDetails>.Update.Set(rec => rec.News, news);
                _context.FindOneAndUpdate(x => x.MemId == mem_id, update);
            }

            //Speeches
            ls = Directory.GetFiles(
                $"C:\\Users\\muhammadS\\Desktop\\majles\\all_members\\speeches"
            );
            foreach (
                var i in ls.Yaap(
                    settings: new YaapSettings
            ...
            )
            {
                ...
                html.Close();
                var mem_id = Path.GetFileName(i).Split(".")[0].toInt32();
                var speeches = hdoc.QuerySelectorAll(".news-item__content")
                    .Select(
                        x =>
                            new News_Speeches
                            {
